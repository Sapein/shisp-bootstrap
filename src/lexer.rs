    ...
    for (li, line) in input.lines().enumerate() {
        for (ci, c) in line.chars().enumerate() {
            ...
            if WHI.is_match(&c.to_string()) && curr_str.chars().count() > 0{
                ...
                tokens.push(Token::new(li, ci-offset, curr_str));
                curr_str = String::new();
                continue;
            }

            if is_special_char(&curr_str, complex_special) && c != '@'{
                tokens.push(Token::new(li, ci - curr_str.chars().count(), curr_str));
                curr_str = String::new();
            }
            ...
            if is_special_char(&c.to_string(), complex_special){
                if curr_str.chars().count() > 1 {
                    ...
                    if ci > 0 {
                        tokens.push(Token::new(li, ci - 1, curr_str));
                    } else {
                        tokens.push(Token::new(li, ci, curr_str));
                    }
                    curr_str = String::new();
                }
            ...
            if kinda_special_char(c) {
                if curr_str.chars().count() > 0 && curr_str.chars().nth(0).unwrap() != ';' && curr_str.chars().nth(0).unwrap() != '"'{
                    let offset = if ci >= curr_str.chars().count() { 
                        curr_str.chars().count() 
                    } else { 
                        0 
        ...
        if curr_str.chars().count() > 0 {
            tokens.push(Token::new(li, line.chars().count() - curr_str.chars().count(), curr_str));
            curr_str = String::new();
        }
    }
    tokens
...
mod tests {
    ...
    fn test_scan_string() {
        ...
        let output = [
            vec![
                Token::new(0, 0, ",@".to_string()),
                Token::new(0, 2, "(".to_string()),
                Token::new(0, 3, "a".to_string()),
                Token::new(0, 5, "b".to_string()),
                Token::new(0, 7, "c".to_string()),
                Token::new(0, 8, ")".to_string())
            ],
            vec![
                Token::new(0, 0, ",".to_string()),
                Token::new(0, 1, "(".to_string()),
                Token::new(0, 2, "a".to_string()),
                Token::new(0, 4, "b".to_string()),
                Token::new(0, 6, "c".to_string()),
                Token::new(0, 7, ")".to_string()),
            ],
            vec![
                Token::new(0, 0, "(".to_string()),
                Token::new(0, 1, "a@b".to_string()),
                Token::new(0, 5, "c".to_string()),
                Token::new(0, 6, ")".to_string()),
            ],
            vec![
                Token::new(0, 0, "(".to_string()),
                Token::new(0, 1, "a".to_string()),
                Token::new(0, 2, ",".to_string()),
                Token::new(0, 3, "b".to_string()),
                Token::new(0, 5, "c".to_string()),
                Token::new(0, 6, ")".to_string()),
            ],
            vec![
                Token::new(0, 0, "\"test\"".to_string())
            ],
            vec![
                Token::new(0, 0, "1234".to_string())
            ],
            vec![
                Token::new(0, 0, "0123".to_string())
            ],
            vec![
                Token::new(0, 0, ";comment".to_string())
            ],
            vec![
                Token::new(0, 0, "123".to_string()),
                Token::new(1, 0, "345".to_string())
            ],
            vec![
                Token::new(0, 0, ",".to_string()),
                Token::new(0, 1, "123".to_string())
            ],
            vec![
                Token::new(0, 0, ",@".to_string()),
                Token::new(0, 2, "123".to_string())
            ]
        ];
